...
class ControllerFeedRestApi extends RestController
{
    ...
    */
    ...
    private function getProductInfo($product, $simpleList = false, $customFields = false)
    {
        ...
        $lp_data = \liveopencart\ext\liveprice::getInstance($this->registry)->getProductPageAdditionalData();
        ...
        $this->parent_child_options_common->addProductPageScripts();
                ...
                foreach ($option['product_option_value'] as $option_value) {
                    if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
                        if ((($this->customer->isLogged() && $this->config->get('config_customer_price')) || !$this->config->get('config_customer_price')) && (float)$option_value['price']) {
                            ...
                            $price = \liveopencart\ext\liveprice::getInstance($this->registry)->changeOptionPriceFormat($price, $option_value);
        ...
        $retval = array(
            ...
            'description' => preg_replace('/style="([a-z]|[A-Z]|[0-9]|-|:| |;|[(]|,|[)]|&|\.)*"/', '' , html_entity_decode( $product['description'], ENT_QUOTES, 'UTF-8') ),
        ...
        $related_options = !empty($pcop_data) ? $pcop_data : array();

        foreach($related_options as $key => $related_option){
            if (empty($related_option)){
                unset($related_options[$key]);
            } else {
                $options = [];
                foreach($related_option as $option){
                    $option['mandatory'] = $option['pcop_or'];
                    unset($option['pcop_or']);
                    unset($option['pcop_id']);
                    $options[] = $option;
                }
                $related_options[$key] = $options;

            }
        }

        $retval['related_options'] = $related_options;

        $related_product_results = $this->model_catalog_product->getProductRelated($product['product_id']);
        $related_product = [];
        foreach ($related_product_results as $result) {
            ...
            $related_product[] = array(
                ...
                'price' =>     $this->currency->format($this->tax->calculate($result['price'], $result['tax_class_id'], $this->config->get('config_tax')),
                    $this->currency->getRestCurrencyCode(),
                    '',
                    false),
        ...
        $retval['related_products'] = $related_product;
...
//            );
    ...
    public function getHomePageCategoryWithItProducts(){
        $this->checkPlugin();
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            $this->json['data'] = [];
            $this->json['data'] = [
                'offer_discount_products' => $this->getCategoryById($this->config->get('config_offer_discount')),
                'latest_products' => $this->getCategoryById($this->config->get('config_latest_product'))
            ];
        } else {
            $this->statusCode = 405;
            $this->allowedHeaders = array("GET");
    ...
    private function getCategoryById($category_id,$level = 10){
        $this->load->model('catalog/category');
        $this->load->model('tool/image');
        ...
        if (isset($category['category_id'])) {
            ...
            $category = array(
                ...
                'products' => $this->getProductCategoryById($category['category_id'],$this->request)
            );
            return $category;
        } else {
            $this->statusCode = 404;
    ...
    public function getProductCategoryById($category_id, $request)
    {
        ...
        if (!empty($products)) {
            foreach ($products as $product) {
                $data[] = $this->getProductInfo($product, $simpleList, $customFields);
            }
        }
            ...
            foreach ($filter_groups as $filter_group) {
                ...
                foreach ($filter_group['filter'] as $filter) {
                    ...
                    if (!empty($filter['image']) && file_exists(DIR_IMAGE . $filter['image'])) {
                        $image = $this->model_tool_image->resize($filter['image'], $this->config->get('config_shopping_cart_rest_api_image_width'), $this->config->get('config_shopping_cart_rest_api_image_height'));
    ...
    public function loadCatTree($parent = 0, $level = 1)
    {
        ...
        if ($categories && $level > 0) {
            ...
            foreach ($categories as $category) {
                $product_count = $this->model_catalog_product->getProductsTotal(['filter_category_id' =>$category['category_id'],'filter_sub_category' => true],$this->customer);
            ...
            foreach ($categories as $category) {
                ...
                if (isset($categoryDetails['category_id'])) {
                    ...
                    $info = array(
                        ...
                        'name' => html_entity_decode($categoryDetails['name'], ENT_QUOTES, 'UTF-8'),
        ...
        if (!empty($result) && empty($this->json['error'])) {
            ...
            foreach ($result as $setting) {
                switch ($setting['key']) {
                    ...
                    case 'config_latest_product':
                        $this->json['data']['config_latest_product'] = $setting['value'];
                        break;
                    case 'config_offer_discount':
                        $this->json['data']['config_offer_discount'] = $setting['value'];
                        break;
                        
                        
                    case 'config_facebook':
                        $this->json['data']['config_facebook'] = $setting['value'];
                        break;
                        
                    case 'config_twitter':
                        $this->json['data']['config_twitter'] = $setting['value'];
                        break;
                    case 'config_instagram':
                        $this->json['data']['config_instagram'] = $setting['value'];
                        break;
                    case 'config_whatsapp':
                        $this->json['data']['config_whatsapp'] = $setting['value'];
                        break;
                }
...
//            );
    ...
    private function getCountryInfo($country_info, $addZone = true)
    {
            ...
            if (isset($country_info['image']) && !empty($country_info['image']) && file_exists(DIR_IMAGE . $country_info['image'])) {
        $image = $this->model_tool_image->resize($country_info['image'], $this->config->get('config_shopping_cart_rest_api_image_width'), $this->config->get('config_shopping_cart_rest_api_image_height'));
        $original_image = $this->urlPrefix . 'image/' . $country_info['image'];
    } else {
        $image = $this->model_tool_image->resize('no_image.png', $this->config->get('config_shopping_cart_rest_api_image_width'), $this->config->get('config_shopping_cart_rest_api_image_height'));
    }
        ...
        if (ctype_digit($id)) {
            ...
            if ($information_info) {
                ...
                $data = [
                    ...
                    'description' => preg_replace('/((style|dir)?="([a-z]|[A-Z]|[0-9]|-|,|;| |=|:|\.|[(]|[)])*"|  |\\r|\\t|\\n)/', '' , html_entity_decode( $information_info['description']) )
                ];
...
//            );
    ...
    public function banners()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            if (isset($this->request->get['key'])) {
                $this->getBanner($this->request->get['key']);
            } else {
                //get banner list
                $this->listBanners();
            }
    ...
    public function getBanner($key)
    {
        ...
        $results = $this->model_design_banner->getBannerByKey($key);
        if (!empty($results)) {
            foreach ($results as $result) {
                if (is_file(DIR_IMAGE . $result['image'])) {
                    $this->json['data'][] = array(
                    ...
                    $this->json['data'][] = array(
                        'title' => $result['title'],
                        'link' => $result['link'],
                        'image' => $this->urlPrefix . 'image/no_image.png'
                    );
                }
            }
...
//            );
    ...
    public function price()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($post['option_oc'])) {
                $options = $post['option_oc'];
            } elseif (isset($post['option'])) {
                $options = $post['option'];
            } else {
                $options = array();
            }
            ...
            if (!empty($post['quantity_per_option']) && is_array($post['quantity_per_option'])) {
                // specific calculation for a specific options (quantity is set for each option value)
                $quantity_per_options = $post['quantity_per_option'];
    ...
    public function getProductPriceWithHtml($product_id, $current_quantity = 0, $options = array(), $multiplied_price = false)
    {
        ...
        $lowest_price = $this->liveopencart_ext_liveprice->getCalc()->getLowestPossiblePriceForProductPageIfEnabled($product_id, $simple_prices['price'], $simple_prices['special'], $simple_prices['tax'], $options);
        if ($lowest_price) {
        ...
        $prices = $this->getPriceHtmls($simple_prices, $product_id);
...
//        }
    ...
    protected function getFinalPricesForOptionValues($product_id, $current_quantity, $options, $multiplied_price)
    {
        ...
        if ($options && !empty($lp_settings['calc_prices_for_options'])) {
            ...
            //foreach ( $options as $po_id => $po_value ) {
            foreach ($product_options as $product_option) {

                if (!empty($product_option['product_option_value'])) {
                    $po_id = $product_option['product_option_id'];
                    //if ( !empty($product_options_assoc[$po_id]) && !empty($product_options_assoc[$po_id]['product_option_value']) ) {
                    //$product_option = $product_options_assoc[$po_id];
                ...
                //$current_options[$po_id] = $po_value;
            }
    ...
    private function getPriceHtmls($prices, $product_id)
    {
...
//        return $htmls;
    ...
    private function getProductTotalPriceForQuantityPerOptionWithHtml($p_product_id, $p_options, $p_quantity_per_options)
    {
        ...
        if ($total_quantity) {
            ...
            return $this->getPriceHtmls($simple_prices, $p_product_id);
