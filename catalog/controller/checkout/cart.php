...
class ControllerCheckoutCart extends Controller {
    public function index() {
        ...
        if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
            ...
            foreach ($products as $product) {
                ...
                $cart = $this->cart->product($product['product_id']);
    ...
    public function add() {
        ...
        $product_info = $this->model_catalog_product->getProductForFeature($product_id);
        ...
        if ($product_info) {
            ...
            if (!$json) {
                ...
                $json['html_cart'] = $this->load->controller('common/cart');
                $json['popup'] = $this->info($this->request->post['product_id']);
...
//			    $productId = $this->cart->update($this->request->post['key'], $this->request->post['quantity']);
//			}
            $productId = $this->cart->update($this->request->post['key'], $this->request->post['quantity']);
            ...
            $json['html_cart'] = $this->load->controller('common/cart');
            $json['popup'] = $this->info($productId);
    ...
    public function info($productId)
    {
        ...
        $cart = $this->cart->product($productId);
        ...
        return $this->load->view('product/product_ajax', $data);
    }
        ...
        if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
            ...
            foreach ($products as $product) {
                ...
                $data['products'][] = array(
                    ...
                    'quantity' => !empty($product['quantity'])?$product['quantity']: 1,
                    'action' => !empty($cart['quantity'])?'edit': 'add',
                    'cart_id' => !empty($product['cart_id'])? $product['cart_id']: 0,
                    'special' => $special,
                    'stock'     => $product['stock'] ? true : !(!$this->config->get('config_stock_checkout') || $this->config->get('config_stock_warning')),
                    'total' => $total,
                    'href' => $this->url->link('product/product', 'product_id=' . $product_info['product_id'])
            ...
            $json['html'] = $this->load->view('checkout/cart_table', $data);
            $this->response->addHeader('Content-Type: application/json');
            $this->response->setOutput(json_encode($json));
        } else {
            $json['reload'] = true;
