...
class ModelAccountOrder extends Model {
	...
	public function getOrder($order_id) {
		$order_query = $this->db->query("SELECT * , os.name as status FROM `" . DB_PREFIX . "order` as o LEFT JOIN " . DB_PREFIX . "order_status os ON (o.order_status_id = os.order_status_id) WHERE order_id = '" . (int)$order_id . "' AND  customer_id = '" . (int)$this->customer->getId() . "' AND customer_id != '0' AND o.order_status_id > '0' AND os.language_id = '" . (int)$this->config->get('config_language_id') . "'");
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				// 'total'                   => round($order_query->row['total']),
				'total'          => $this->currency->format(round($order_query->row['total']), $order_query->row['currency_code'], $order_query->row['currency_value']),
    ...
    public function getOrderShippingCost($order_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_total WHERE code LIKE 'shipping' AND order_id = '" . (int)$order_id . "' ORDER BY sort_order");
        return $query->row;
    }
