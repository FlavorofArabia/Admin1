...
class liveprice extends \liveopencart\lib\v0020\extension {
	...
	public function getOptionValuePriceByProductCurrency($option_value, $product_id    = false) {
		...
		if (!empty($option_value['price']) && !in_array($option_value['price_prefix'], $this->getMultiplyingPricePrefixes())) {
			...
			$product_query = $this->getCalc()->getProductQuery($product_id);
			if ($product_query->num_rows && !empty($product_query->row['currency_id'])) {
				$price         = $this->getCalc()->convertFromCurrency($option_value['price'], $product_query->row['currency_id']);
			}
		}
		return $price;
	}
