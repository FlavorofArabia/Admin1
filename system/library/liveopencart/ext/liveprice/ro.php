...
class ro extends \liveopencart\lib\v0020\sub_library {
  
	use \liveopencart\lib\v0020\traits\cache;
	use \liveopencart\lib\v0020\traits\installed;
	
	public function installed() {
		return $this->getExtensionInstalledStatus('related_options', 'ro_installed', function(){
			return file_exists( DIR_APPLICATION . 'model/' . 'extension/liveopencart/related_options' . '.php' );
		});
	}
	...
	public function getProductStartingFromOptions($product_id, $io_defaults, $product_price, $sql_min_options) {
		...
		if ( $this->installed() ) {
			...
			$ro_ids = false;
			if ( $io_defaults ) {
				$ro_io_combs = $this->getModule()->getROCombsByPOIdsAssocParams(array('product_id'=>$product_id, 'options'=>$io_defaults, 'strict'=>false));
			...
			if ( !empty($ro_settings['spec_price_special']) ) {
				...
				$sql_basic_left_join.= " LEFT JOIN ".DB_PREFIX."relatedoptions_special ROS
						ON (ROS.relatedoptions_id = RO.relatedoptions_id AND ROS.customer_group_id = ".(int)$this->config->get('config_customer_group_id').") ";
			}
