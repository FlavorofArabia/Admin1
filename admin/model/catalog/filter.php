...
class ModelCatalogFilter extends Model {
	public function addFilter($data) {
		...
		foreach ($data['filter_group_description'] as $language_id => $value) {
			...
			filter_group_id = '" . (int)$filter_group_id . "', language_id = '" . (int)$language_id . "',
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				...
				foreach ($filter['filter_description'] as $language_id => $filter_description) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "filter_description SET filter_id = '" . (int)$filter_id . "', language_id = '" . (int)$language_id . "', filter_group_id = '" . (int)$filter_group_id . "',
	...
	public function editFilter($filter_group_id, $data) {
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				if ($filter['filter_id']) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET filter_id = '" . (int)$filter['filter_id'] . "', filter_group_id = '" . (int)$filter_group_id . "',
					    image = '" . (string)$filter['image'] . "',
